...
var Storage = /** @class */ (function () {
    ...
    Storage.prototype.configure = function (config) {
        ...
        Object.keys(storageConfig).forEach(function (providerName) {
            if (typeof storageConfig[providerName] !== 'string') {
                _this._config[providerName] = tslib_1.__assign(tslib_1.__assign({}, _this._config[providerName]), storageConfig[providerName]);
            }
        });
    ...
    Storage.prototype.copy = function (src, dest, config) {
        ...
        if (typeof plugin.copy !== 'function') {
            return Promise.reject(".copy is not implemented on provider " + plugin.getProviderName());
        }
        var responsePromise = plugin.copy(src, dest, tslib_1.__assign(tslib_1.__assign({}, config), { cancelTokenSource: cancelTokenSource }));
    ...
    Storage.prototype.get = function (key, config) {
        ...
        var cancelTokenSource = this.getCancellableTokenSource();
        var responsePromise = plugin.get(key, tslib_1.__assign(tslib_1.__assign({}, config), { cancelTokenSource: cancelTokenSource }));
        this.updateRequestToBeCancellable(responsePromise, cancelTokenSource);
        return responsePromise;
    };
    ...
    Storage.prototype.getProperties = function (key, config) {
        ...
        if (typeof plugin.getProperties !== 'function') {
            return Promise.reject(".getProperties is not implemented on provider " + plugin.getProviderName());
        }
        var responsePromise = plugin === null || plugin === void 0 ? void 0 : plugin.getProperties(key, tslib_1.__assign({}, config));
        this.updateRequestToBeCancellable(responsePromise, cancelTokenSource);
        return responsePromise;
    };
    Storage.prototype.put = function (key, object, config) {
        ...
        var cancelTokenSource = this.getCancellableTokenSource();
        var response = plugin.put(key, object, tslib_1.__assign(tslib_1.__assign({}, config), { cancelTokenSource: cancelTokenSource }));
...
var getInstance = function () {
    ...
    _instance.configure = function (options) {
        ...
        var vaultConfig = tslib_1.__assign({}, old_configure.call(_instance, options));
        // set level private for each provider for the vault
        Object.keys(vaultConfig).forEach(function (providerName) {
            if (typeof vaultConfig[providerName] !== 'string') {
                vaultConfig[providerName] = tslib_1.__assign(tslib_1.__assign({}, vaultConfig[providerName]), { level: 'private' });
            }
        });
